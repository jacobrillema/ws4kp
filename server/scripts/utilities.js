...
const utils = (() => {
	...
	const MphToKph = (Mph) => Math.round(parseFloat(Mph) * 1.60934);
	const KphToMph = (Kph) => Math.round(parseFloat(Kph) / 1.60934);
	const CelsiusToFahrenheit = (Celsius) => Math.round(parseFloat(Celsius) * 9 / 5 + 32);
	const FahrenheitToCelsius = (Fahrenheit) => Math.round2(((parseFloat(Fahrenheit) - 32) * 5) / 9, 1);
	const MilesToKilometers = (Miles) => Math.round(parseFloat(Miles) * 1.60934);
	const KilometersToMiles = (Kilometers) => Math.round(parseFloat(Kilometers) / 1.60934);
	const FeetToMeters = (Feet) => Math.round(parseFloat(Feet) * 0.3048);
	const MetersToFeet = (Meters) => Math.round(parseFloat(Meters) / 0.3048);
	const InchesToCentimeters = (Inches) => Math.round2(parseFloat(Inches) * 2.54, 2);
	const PascalToInHg = (Pascal) => Math.round2(parseFloat(Pascal)*0.0002953,2);
